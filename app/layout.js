...
export default function RootLayout({ children }) {
  return (
    <html lang="en">
      <body className="min-h-screen bg-[#F2EBE4] text-[#1c1917] flex flex-col justify-between py-12 px-8 md:py-20 md:px-24 font-sans antialiased selection:bg-amber-200 relative overflow-x-hidden isolate">
        ...
        <header className="w-full max-w-5xl mx-auto flex flex-col md:flex-row md:justify-between md:items-start z-10 relative pt-12 pb-3 gap-6 md:gap-0">
          ...
          <nav className="flex items-center gap-8 text-xs text-stone-800 font-mono tracking-tight font-semibold md:pt-1">
            ...
            <Link href="/contact" className="hover:text-stone-950 transition-colors relative after:absolute after:bottom-[-2px] after:left-0 after:w-0 after:h-[1px] after:bg-amber-800 hover:after:w-full after:transition-all">
              contact
            </Link>
          </nav>
        </header>

        {/* Dynamic Page Content Injector — mt-0 tightens the gap below the rule */}
        <main className="w-full max-w-5xl mx-auto flex-1 flex flex-col justify-center z-10 relative mt-0 mb-12 isolate">
          {children}
        </main>

        {/* Global Bottom Footer */}
        <footer className="w-full max-w-5xl mx-auto flex justify-between items-center pt-6 z-10 text-xs text-stone-700 font-mono">
