...
export default function RootLayout({ children }) {
  return (
    <html lang="en">
      ...
      <body className={`antialiased min-h-screen bg-[#F2EBE4] text-[#1c1917] flex flex-col justify-between py-12 px-8 md:py-20 md:px-24 selection:bg-amber-200 relative overflow-x-hidden isolate ${bodyFont.className}`}>
        ...
        <header className="w-full max-w-5xl mx-auto flex flex-col md:flex-row md:justify-between md:items-start z-10 relative pt-12 pb-3 gap-6 md:gap-0">
          ...
          />
          <style dangerouslySetInnerHTML={{__html: `@keyframes gradientFlow { 0% { background-position: 0% 50%; } 50% { background-position: 100% 50%; } 100% { background-position: 0% 50%; } }`}} />
          ...
          <nav className="flex items-center gap-8 text-xs text-stone-800 tracking-tight font-semibold md:pt-1">
            <Link href="/" className="hover:text-stone-950 transition-colors">work</Link>
